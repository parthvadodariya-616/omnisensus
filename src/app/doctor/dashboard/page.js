...
export default function DoctorDiagnostic() {
  ...
  useEffect(() => {
    ...
    api.get('/patients?page_size=50')
      .then(r => {
        ...
        setPatients(list);
        // Do not setSel here; let a separate effect handle it
      })
      .catch(() => {})
      .finally(() => setLoadingPatients(false));
  }, []);

  // Set default selected patient only after patients are loaded
  useEffect(() => {
    if (!sel && patients.length) {
      setSel(patients[0].patient_id);
    }
  }, [patients, sel]);
  ...
  // Fade out loader before unmounting
  useEffect(() => {
    if (!loadingVitals && !loadingPatients) {
      loaderTimeout.current = setTimeout(() => setShowLoader(false), 350); // match CSS transition
    } else {
      setShowLoader(true);
    }
    ...
    <div>
      {showLoader && (
        <div className="page-inner" style={{ paddingTop: 30 }}>
          <div className={`page-loader${(!loadingVitals && !loadingPatients) ? ' hidden' : ''}`}>
            <div className="spin-sm" />
            <div className="page-loader__text">Loading patient vitals...</div>
          </div>
        </div>
      )}

      {!showLoader && !loadingVitals && !loadingPatients && !vitals && (
        <div className="page-inner" style={{ paddingTop: 30 }}>
                  ...
                  <div className="wizard-grid">
                    ...
                    <div className="wiz-group">
                      ...
                      <input type="number" className="wiz-input" value={vitals.bmi} step={0.1} min={10} max={60}
                        onChange={e => upd('bmi', e.target.value)} />
                    </div>
                    <div className="wiz-group">
