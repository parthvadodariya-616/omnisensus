...
const DOCTOR_ROUTES = new Set([
  '/doctor', '/doctor/dashboard', '/doctor/lab',
  '/doctor/patients', '/doctor/reports',
  '/doctor/schedule', '/doctor/notifications', '/doctor/settings',
]);

const NAV = [
  { group: 'Clinical', items: [
    { label: 'Diagnostic Engine', href: '/doctor/dashboard',
    ...
    { label: 'AI Lab',            href: '/doctor/lab',
      icon: <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M9 3H5a2 2 0 0 0-2 2v4m6-6h10a2 2 0 0 1 2 2v4M9 3v18m0 0h10a2 2 0 0 0 2-2V9M9 21H5a2 2 0 0 1-2-2V9m0 0h18"/></svg> },
...
export default function DoctorLayout({ children }) {
  ...
  useEffect(() => {
    const u = getUser();
    if (!u || u.role !== 'doctor') { router.replace('/auth'); return; }
    setUser(u);
  }, [router]);

  useEffect(() => {
    if (pathname === '/doctor/profile') { router.replace('/doctor/settings'); return; }
    if (pathname?.startsWith('/doctor') && !DOCTOR_ROUTES.has(pathname)) {
  ...
  return (
    <div>
      <nav id="nav">
        ...
        <a className="nav-brand" href="/doctor/dashboard" style={{ textDecoration: 'none' }}>
          <Logo size={28} />
        </a>
        ...
        <div className="nav-right">
          ...
          <div className="nav-avatar nav-avatar--doctor">{initials}</div>
          <div className="nav-user-info" style={{ cursor: 'pointer' }}
            onClick={() => router.push('/doctor/settings')}>
            <span>{user?.name || user?.username || 'Doctor'}</span>
